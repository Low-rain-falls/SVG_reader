...
public:
  my_ellipse(string name, string transform, double cx = 0, double cy = 0,
             double rx = 1, double ry = 1, Color stroke = Color(0, 0, 0),
  ...
  void render(Graphics &graphic, vector<LinearGradient> gradients) override {
    if (fill[0] == 'u' && fill[1] == 'r' && fill[2] == 'l') {
      ...
      graphic.FillEllipse(brush, static_cast<REAL>(this->cx - this->rx),
                          static_cast<REAL>(this->cy - this->ry),
                          static_cast<REAL>(this->rx * 2),
                          static_cast<REAL>(this->ry * 2));
      graphic.DrawEllipse(&pen, static_cast<REAL>(this->cx - this->rx),
                          static_cast<REAL>(this->cy - this->ry),
                          static_cast<REAL>(this->rx * 2),
                          static_cast<REAL>(this->ry * 2));
      return;
    ...
    Color stroke_color(255 * this->stroke_opacity, this->stroke.GetR(),
                       this->stroke.GetG(), this->stroke.GetB());
    Color fill_color(255 * this->fill_opacity, filll.GetR(), filll.GetG(),
                     filll.GetB());
