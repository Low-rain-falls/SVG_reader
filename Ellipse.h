...
class my_ellipse : public SVGElement
{
	private:
		...
		string transform;
	public:
		my_ellipse(string name, string transform, int cx = 0, int cy = 0, int rx = 1, int ry = 1, Color stroke = Color(0, 0, 0), double stroke_width = 1, double stroke_opacity = 1, Color fill = Color(0, 0, 0), double fill_opacity = 1) : cx(cx), cy(cy), rx(rx), ry(ry), stroke(stroke), fill(fill), stroke_width(stroke_width), stroke_opacity(stroke_opacity), fill_opacity(fill_opacity), SVGElement(name), transform(transform) {}
