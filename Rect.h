...
class my_rect : public SVGElement
{
	private:
		...
		string transform;
public:
	my_rect(string name, string transform, double x = 0, double y = 0, double width = 1, double height = 1, Color stroke = Color(0,0,0), double stroke_width = 1, double stroke_opacity = 1, Color fill = Color(0, 0, 0), double fill_opacity = 1) : x(x), y(y), width(width), height(height), stroke(stroke), fill(fill), stroke_width(stroke_width), stroke_opacity(stroke_opacity), fill_opacity(fill_opacity), SVGElement(name), transform(transform) {}
