...
public:
  my_circle(string name, string transform, int cx = 0, int cy = 0, int r = 1,
            Color stroke = Color(0, 0, 0), double stroke_width = 1,
  ...
  void render(Graphics &graphic) override {
    if ((this->fill.GetR() != 255 && this->fill.GetG() != 255 &&
         this->fill.GetB() != 255)) {
      ...
      graphic.FillEllipse(&brush, this->cx - this->r, this->cy - this->r,
                          this->r * 2, this->r * 2);
      graphic.DrawEllipse(&pen, this->cx - this->r, this->cy - this->r,
                          this->r * 2, this->r * 2);
    } else {
      Color strokeColor(255 * this->stroke_opacity, this->stroke.GetR(),
                        this->stroke.GetG(), this->stroke.GetB());
