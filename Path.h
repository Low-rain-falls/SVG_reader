...
public:
  my_path(string name, string transform, vector<char> commands, double *points,
          int num_points, Color fill_color, double fill_opacity,
          Color stroke_color, double stroke_width);
  void render(Graphics &graphics) override;
  string getTransform() override { return transform; }
};

my_path::my_path(string name, string transform, vector<char> commands,
                 double *points, int num_points, Color fill_color,
                 double fill_opacity, Color stroke_color, double stroke_width)
    : commands(commands), points(points), num_points(num_points),
...
void my_path::render(Graphics &graphics) {
  ...
  for (int i = 0; i < commands.size(); i++) {
    ...
    case 'm':
      ...
      break;
    case 'C':
      path.AddBezier(endPoint1, endPoint2, points[control_index],
                     points[control_index + 1], points[control_index + 2],
                     points[control_index + 3], points[control_index + 4],
                     points[control_index + 5]);
      endPoint1 = points[control_index + 4];
      endPoint2 = points[control_index + 5];
      control_index += 6;
      break;
    case 'c':
      path.AddBezier(endPoint1, endPoint2, endPoint1 + points[control_index],
                     endPoint2 + points[control_index + 1],
                     endPoint1 + points[control_index + 2],
                     endPoint2 + points[control_index + 3],
                     endPoint1 + points[control_index + 4],
                     endPoint2 + points[control_index + 5]);
      endPoint1 += points[control_index + 4];
      endPoint2 += points[control_index + 5];
      ...
      break;
    case 'L':
      path.AddLine(endPoint1, endPoint2, points[control_index],
                   points[control_index + 1]);
      endPoint1 = points[control_index];
      endPoint2 = points[control_index + 1];
      control_index += 2;
      break;
    ...
    case 'V':
      ...
      break;
    case 'v':
      path.AddLine(endPoint1, endPoint2, endPoint1,
                   endPoint2 + points[control_index]);
      endPoint2 += points[control_index];
      control_index++;
      break;
