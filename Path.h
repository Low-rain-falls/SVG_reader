		...
		for (int i = 0; i < length; i++) {
			...
			case 'A': {
				...
				for (int j = 0; j < numPoints; j += 5) {
					...
					REAL startAngle = atan2(-ry, rx) * 180 / M_PI; // Góc từ trục x đến điểm bắt đầu
					REAL sweepAngle = sweep ? 180 : -180;         // Sweep clockwise hoặc counter-clockwise
					if (largeArc) {
						sweepAngle = sweep ? 360 - abs(sweepAngle) : -(360 - abs(sweepAngle));
					}
			...
			case 'a': {
				...
				for (int j = 0; j < numPoints; j += 5) {
					...
					bool sweep = static_cast<bool>(get<1>(d[i])[j + 3].X); // sweep-flag
					PointF startPoint = endPoint; // Điểm bắt đầu
					PointF relativeEndPoint = get<1>(d[i])[j + 4]; // Điểm kết thúc (relative)

					// Tính toán điểm kết thúc tuyệt đối từ điểm bắt đầu
					PointF absoluteEndPoint = PointF(
