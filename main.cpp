...
vector<SVGElement*> parseSVG(string filePath, vector<double>& boxValues, string& widthS, string& heightS) {
	...
	if (svgNode) {
		if (string(svgNode->first_node()->name()) == "defs" || string(svgNode->first_node()->name()) == "linearGradient") {
			...
			for (rapidxml::xml_node<>* node = gradientNode->first_node(); node; node = node->next_sibling())
			{
				if (string(node->name()) == "linearGradient") {
					string id = gradientNode->first_attribute("id")->value();
					double x1 = stod(gradientNode->first_attribute("x1")->value());
					double y1 = stod(gradientNode->first_attribute("y1")->value());
					double x2 = stod(gradientNode->first_attribute("x2")->value());
					double y2 = stod(gradientNode->first_attribute("y2")->value());
