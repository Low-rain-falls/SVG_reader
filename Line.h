...
myLine::myLine(int x1, int y1, int x2, int y2, Color stroke, int strokeWidth,
               float strokeOpacity) {
  this->x1 = x1;
  this->y1 = y1;
  this->x2 = x2;
  this->y2 = y2;
  // this->stroke.SetValue(Color(strokeOpacity * 255, stroke.GetR(),
  // stroke.GetG(), stroke.GetB()));
  this->stroke =
      Color(strokeOpacity * 255, stroke.GetR(), stroke.GetG(), stroke.GetB());
